...
var uploadedFiles = {};        // path -> { path, file, kind }
var uploadOrder = [];          // ترتیب اولین‌بار آپلود شدن، برای نمایش
var uploadJsManualOrder = [];  // ترتیب دستی فایل‌های js، فقط وقتی HTML وجود نداره استفاده می‌شه
var uploadBlobUrls = {};       // path -> object URL (هر اجرا از نو ساخته می‌شه)
...
function uploadAddFileArray(items) {
  items.forEach(function (item) {
    if (!item.file || !item.path) return;
    uploadUpsertEntry(item.path, item.file);
  });
  uploadRender();
}
...
function uploadRender() {
  ...
  var showJsArrows = !htmlEntry;

  uploadHintText.textContent = htmlEntry
    ? ('ورودی: ' + htmlEntry.path + ' — مسیر و ترتیب فایل‌ها از خودش خونده می‌شه')
    : 'بدون HTML — فایل‌های JS به این ترتیب اجرا می‌شن (با ▲▼ عوضش کن)';
...
function uploadRevokeBlobUrls() {
  Object.keys(uploadBlobUrls).forEach(function (p) {
    try { URL.revokeObjectURL(uploadBlobUrls[p]); } catch (e) { /* ignore */ }
  });
  uploadBlobUrls = {};
}

function uploadTextMime(kind) {
  if (kind === 'js') return 'text/javascript';
  if (kind === 'css') return 'text/css';
  return 'text/plain';
}

function uploadBuildBlobMap() {
  uploadRevokeBlobUrls();
  var textReads = [];
  uploadOrder.forEach(function (path) {
    var entry = uploadedFiles[path];
    if (entry.kind === 'js' || entry.kind === 'css') {
      textReads.push(entry.file.text().then(function (text) {
        var blob = new Blob([text], { type: uploadTextMime(entry.kind) });
        uploadBlobUrls[path] = URL.createObjectURL(blob);
      }));
    } else if (entry.kind !== 'html') {
      uploadBlobUrls[path] = URL.createObjectURL(entry.file);
    }
  });
  return Promise.all(textReads);
}

function uploadResolveAttr(el, attr, baseDir, usedPaths) {
  var raw = el.getAttribute(attr);
  if (!raw) return;
  if (/^([a-z][a-z0-9+.-]*:)?\/\//i.test(raw) || /^data:/i.test(raw) || /^#/.test(raw) || /^(mailto|tel|javascript):/i.test(raw)) return;

  var key = uploadResolvePath(baseDir, raw);
  if (uploadBlobUrls[key]) {
    el.setAttribute(attr, uploadBlobUrls[key]);
    usedPaths[key] = true;
    return;
  }
  var flatKey = raw.replace(/^\.?\//, '');
  if (uploadBlobUrls[flatKey]) {
    el.setAttribute(attr, uploadBlobUrls[flatKey]);
    usedPaths[flatKey] = true;
  }
}
...
function uploadBuildDocFromHtml(htmlEntry) {
  return htmlEntry.file.text().then(function (htmlText) {
    ...
    doc.querySelectorAll('script[src]').forEach(function (el) { uploadResolveAttr(el, 'src', baseDir, usedPaths); });
    doc.querySelectorAll('link[rel="stylesheet"][href]').forEach(function (el) { uploadResolveAttr(el, 'href', baseDir, usedPaths); });
    doc.querySelectorAll('img[src]').forEach(function (el) { uploadResolveAttr(el, 'src', baseDir, usedPaths); });
    doc.querySelectorAll('source[src]').forEach(function (el) { uploadResolveAttr(el, 'src', baseDir, usedPaths); });
    doc.querySelectorAll('video[src], audio[src]').forEach(function (el) { uploadResolveAttr(el, 'src', baseDir, usedPaths); });
...
function uploadBuildDocFromFilesOnly() {
  ...
  cssPaths.forEach(function (p) {
    if (uploadBlobUrls[p]) parts.push('<link rel="stylesheet" href="' + uploadBlobUrls[p] + '">');
  });
  parts.push('</head><body>');
  jsPaths.forEach(function (p) {
    if (uploadBlobUrls[p]) parts.push('<' + 'script src="' + uploadBlobUrls[p] + '"></' + 'script>');
  });
...
function runUpload() {
  ...
  uploadBuildBlobMap()
    .then(function () {
      ...
      return htmlEntry ? uploadBuildDocFromHtml(htmlEntry) : Promise.resolve(uploadBuildDocFromFilesOnly());
    })
